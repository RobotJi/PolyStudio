...
- **统一 Mock 模式**：3D/图片/视频工具统一 Mock 模式，便于离线或调试 🆕
...
- **无限画板**：生成的图片自动插入画布，支持缩放/对齐/框选/编辑等
- **3D 模型查看器**：前端集成 3D 模型预览功能，支持 OBJ/GLB 格式
- **项目管理**：项目列表、重命名、复制链接、删除
- **全局深色主题**：前端与画板默认深色
- **本地持久化**：图片保存到 `backend/storage/images/`，视频保存到 `backend/storage/videos/`，3D 模型保存到 `backend/storage/models/`（包含 OBJ、MTL、纹理文件），项目与聊天记录保存到 `backend/storage/chat_history.json`
